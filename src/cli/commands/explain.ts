...
export async function explainCommand(
  ...
  try {
    ...
    console.log(
      `This repository appears to be a ${report.frameworks.data.length > 0 ? report.frameworks.data.map((f) => f.name).join("/") : "codebase"} project.`
    );
    console.log(`\nKey findings:`);
    ...
    if (report.infrastructure.data.docker || report.infrastructure.data.terraform) {
      console.log(
        `- Infrastructure: ${[report.infrastructure.data.docker && "Docker", report.infrastructure.data.terraform && "Terraform"].filter(Boolean).join(", ")}`
      );
    }
  } catch (err) {
